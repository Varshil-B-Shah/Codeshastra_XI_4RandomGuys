...
import { FaCompass, FaMapMarkedAlt, FaUsers, FaCalculator, FaVrCardboard, FaHashtag, FaBook, FaTrophy } from 'react-icons/fa';
...
const DashboardSidebar = () => {
  ...
  const menuItems = [
    ...
    { name: 'leaderboard', displayName: 'Leaderboard', icon: <FaTrophy className="text-xl" />, path: '/dashboard/leaderboard' },
  ];
  ...
  return (
    <div className="h-screen bg-gradient-to-br from-[#1A2B6D] to-[#3B53AF] text-white w-72 fixed left-0 top-0 overflow-y-auto shadow-lg">
      <div className="py-8 px-6 text-center border-b border-[#4C63B6]/40">
        ...
        <p className="text-sm text-blue-100 mt-1.5 font-light tracking-wide">Your Travel Companion</p>
      </div>
      ...
      <nav className="p-5">
        <div className="space-y-1.5">
          {menuItems.map((item, index) => (
            ...
            >
              <div className={`mr-3.5 ${isActiveRoute(item.path) ? 'text-[#1A2B6D]' : 'text-blue-200'}`}>
                {item.icon}
              </div>
              <span className="text-[15px] tracking-wide">{item.displayName}</span>
            </Link>
          ))}
        </div>
        
        <div className="mt-10 px-3 py-4 bg-[#4C63B6]/20 rounded-lg">
          <p className="text-xs text-blue-100 font-medium mb-3">PREMIUM FEATURES</p>
    ...
    <div className="flex bg-[#F8FAFC] min-h-screen font-['Inter']">
      ...
      <div className="ml-72 flex-1 p-8 pt-6 max-w-[1800px]">
        <header className="mb-8 bg-white p-7 rounded-2xl shadow-sm border border-[#E2E8F0] transition-all hover:shadow-md">
          <h1 className="text-3xl font-bold text-[#1A202C] font-['Poppins']">{title}</h1>
